    ...
    <div id = "contents" style="width: 800px;height: 250px">
        ...
        <div id = "right" style="float: left; width: 50%; height: 40%;">
            ...
            <%
                ...
                commentss = DaoFactory.getBook_commentsDaoInstance().queryByBook_id(bookid);
                users = DaoFactory.getUserDaoInstance().queryByBookCommentsBookId(bookid);
                Book book = null;
                book = DaoFactory.getBookDaoInstance().queryById(bookid);
            %>
            书名：
            <%=book.getName()%>
            <br><br>
            ...
            <%=book.getBorrow_num()%>
            次<br><br>
            <form action="borrowbook.do?bookid=<%=bookid%>" method="post">
...
//                List<User> users1 = new ArrayList<User>();
                ...
                String comment = null;
//                users1 = DaoFactory.getUserDaoInstance().queryByBookCommentsBookId(bookid);
                book_commentss = DaoFactory.getBook_commentsDaoInstance().queryByBook_id(bookid);
                for(int i = 0; i < book_commentss.size(); i++) {
                    comment = book_commentss.get(i).getComment_datetime() + " 　 "
                            + DaoFactory.getUserDaoInstance().queryById(book_commentss.get(i).getUser_id()).getName() + ":　 " + book_commentss.get(i).getDetail();
                    comments.add(comment);
                }

            %>

            <marquee behavior = "scroll" direction = "up" behavior = "scroll" height="auto" scrolldelay=300 scrollamount=10 >
                <%
...
<script type="text/javascript">
    ...
    var mark = <%=request.getAttribute("mark")%>;
    if(mark == "1"){
        alert("借阅成功！");
        showDialog();
    }
